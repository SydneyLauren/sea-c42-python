...
def most_recent_poll_row(poll_rows, pollster, state):
    ...
    returndata = 'None'
    checkdate = 'Jan 01 1900'
    for i in range(len(poll_rows)):
        if state in poll_rows[i]['State'] and \
           pollster in poll_rows[i]['Pollster']:
            if earlier_date(checkdate, poll_rows[i]['Date']):
                returndata = poll_rows[i]
        else:
            continue
    if returndata == 'None':
        return None
...
def unique_column_values(rows, column_name):
    ...
    # Make a list of the values corresponding to column names
    col_list = []
    for row in rows:
        col_list.append(row[(column_name)])

    # Reformat/make the list unique using set
    returndata = set(col_list)
    return(returndata)
...
def pollster_predictions(poll_rows):
    ...
    """ I give up. I don't know why this doesn't work."""
    if len(poll_rows) > 1:
        for i in range(len(poll_rows) - 1):
            if poll_rows[i]['Pollster'] == poll_rows[i + 1]['Pollster'] and \
               poll_rows[i]['State'] == poll_rows[i + 1]['State']:
                    poll_rows = ([most_recent_poll_row(poll_rows,
                        poll_rows[i]['Pollster'], poll_rows[i]['State'])])

    edge = state_edges(poll_rows)
    for pr in poll_rows:
        edge = state_edges([pr])
        if pr['Pollster'] in d:
            n = d[(pr['Pollster'])]
            z = dict(list(n.items()) + list(edge.items()))
            d[(pr['Pollster'])] = z
        else:
            d[(pr['Pollster'])] = edge
    return(d)
